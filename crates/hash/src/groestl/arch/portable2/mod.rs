...
/// The context contains the actual state of [compress512::COLS] in addition to the round constants.
/// The idea is the to hope the cache CPU is used more often, and we don't fetch the round constants
/// from the binary too often.
const CONTEXT_SIZE: usize = COLS + 256 * 8;

impl GroestlShortInternal for GroestlShortImpl {
	type State = [u64; CONTEXT_SIZE];

	fn state_from_bytes(block: &[u8; 64]) -> Self::State {
		let mut m = [0; CONTEXT_SIZE];
		for (chunk, v) in block.chunks_exact(8).zip(m.iter_mut()) {
			*v = u64::from_be_bytes(chunk.try_into().unwrap());
		}
		m[COLS..].copy_from_slice(&table::TABLE);
		m
	}

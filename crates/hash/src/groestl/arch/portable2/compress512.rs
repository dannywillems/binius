...
use super::{table::TABLE, GroestlShortImpl, State, COLS, CONTEXT_SIZE, ROUNDS};

#[inline(always)]
fn column(x: &[u64; CONTEXT_SIZE], c: [usize; 8]) -> u64 {
	let mut t = 0;
	for i in 0..8 {
		let sl = 8 * (7 - i);
...
#[inline(always)]
fn rndq(mut x: [u64; CONTEXT_SIZE], r: u64) -> [u64; COLS] {
	for i in 0..COLS {
...
#[inline(always)]
fn rndp(mut x: [u64; CONTEXT_SIZE], r: u64) -> [u64; COLS] {
	for i in 0..COLS {
		x[i] ^= ((i as u64) << 60) ^ r;
	}
	[
...
pub fn compress(h: &mut [u64; CONTEXT_SIZE], block: &[u8; 64]) {
	let mut q = [0u64; COLS];
...
pub fn p(h: &mut [u64; CONTEXT_SIZE]) {
	for i in 0..ROUNDS {
		*h = rndp(*h, i << 56);
	}
}

pub fn q(h: &mut [u64; CONTEXT_SIZE]) {
	for i in 0..ROUNDS {
		*h = rndq(*h, i);
	}
}
